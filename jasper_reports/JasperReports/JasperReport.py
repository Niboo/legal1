...
class JasperReport:
    def __init__(self, fileName='', pathPrefix=''):
        ...
        self._overrideLanguageLocation = False
        if fileName:
    ...
    def overrideLanguageLocation(self):
        return self._overrideLanguageLocation
    ...
    def extractProperties(self):
        ...
        # Override languag        
        copiesTags = doc.xpath( '/jr:jasperReport/jr:property[@name="OPENERP_LANGUAGE"]', namespaces=nss )
        if copiesTags and 'value' in copiesTags[0].keys():
            self._overrideLanguageLocation = copiesTags[0].get('value')
