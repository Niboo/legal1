    ...
    def generateIds(self, record, relations, path, currentRecords):
        ...
        for relation in unrepeated:
            ...
            if root == 'Attachments':
                ...
                if root == 'id':
                    value = record._id
                elif record.__getitem__(root):
                    value = record.__getitem__(root)
                else:
                    self.warning("Field '%s' does not exist in model '%s'." % (root, record._table))
                    continue
                ...
                if not isinstance(value, browse_record_list):
...
class XmlBrowseDataGenerator(BrowseDataGenerator):
    ...
    def generate(self, fileName):
        ...
        for record in self.pool.get(self.model).browse(self.cr, self.uid, self.ids, self.context):
            ...
            copies = 1
            if self.report.copiesField() and record.__getitem__(self.report.copiesField()):
                copies = int( record.__getitem__(self.report.copiesField()) )
    ...
    def generateXmlRecord(self, record, records, recordNode, path, fields):
        ...
        for field in unrepeated:
            ...
            if isinstance(value, browse_record_list):
                if not value:
                    continue
                fields2 = [ f.partition('/')[2] for f in fields if f.partition('/')[0] == root ]
...
class CsvBrowseDataGenerator(BrowseDataGenerator):
    ...
    def generate(self, fileName):
        ...
        overrideLanguageLocation = self.report.overrideLanguageLocation()
        for record in self.pool.get(self.model).browse(self.cr, self.uid, self.ids, self.context):
            if overrideLanguageLocation:
                overrideLanguages = overrideLanguageLocation.split('.')
                lang_code = record
                for overrideLanguage in overrideLanguages:
                    lang_code = lang_code.__getattr__(overrideLanguage)
                self.context.update({'lang': lang_code})
                record = self.pool.get(self.model).browse(self.cr, self.uid, record.id, self.context)
                
            newRecords = self.generateIds( record, relations, '', [ { 'root': record } ] )
            copies = reportCopies
            if copiesField and record.__getitem__(copiesField):
                copies = copies * int( record.__getitem__(copiesField) )
            sequence += 1
            subsequence = 0
            for new in newRecords:
        ...
        for field in unrepeated:
            ...
            if root == 'Attachments':
                ...
                if root == 'id':
                    ...
                    value = None
                    self.warning("Field '%s' (path: %s) does not exist in model '%s'." % (root, currentPath, record._table))
