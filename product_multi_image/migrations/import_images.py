...
def convert(config):
    ...
    logger.warning("**** start import program : %s" % (process_start_time))
    logger.warning("succes creating sockets")
    logger.warning("csvfile %s " % "csvfiles/odoo_import_images.csv")
    logger.warning("**** start import images")
    print 'start import'
    start_time = time.time()
    product_images(e)
    logger.warning("**** stop import images, time used %s" % (time.time() - start_time))

    logger.warning("**** end import program : %s" % (time.time() - process_start_time))
    print 'please consult the import.log file'

def product_images(e):
    t = 0
    ...
    with open("odoo_import_images.csv","rb") as csvfile:
        ...
        for line in reader:
            ...
            data = {
                ...
                'image': line[2],
                'mage_file': line[3],
                'mage_product_id': line[4],
                'types': line[5].split(','),
            }

            try:
                e('product.image', 'create_image_helper', {
                    'mage_product_id': line[4],
                    'product_id': line[0],
                    'product_type': '',
                    'image_list': [data],
                ...
                logger.error("(2) xml_id write %s" % (line[0]))
                logger.error(line)
                logger.error(error)
                continue
